...
def read_csv_and_publish(csv_file, pub):
    with open(csv_file, 'r') as file:
        ...
        gnss_raw_array.header.frame_id = "ECEF"
        while True:
            ...
            if sv_count< total_sv:
                ...
                for row in rows:
                    gnss_raw = GNSS_Raw()
                    gnss_raw.GNSS_time = gnss_time
                    gnss_raw.total_sv = total_sv
...
def main():
    ...
    pub = rospy.Publisher('/gnss_preprocessor_node/GNSSPsrCarRov1', GNSS_Raw_Array, queue_size=25)
